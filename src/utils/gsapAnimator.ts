...
export class GSAPAnimator {
	...
	animateSequence(configs: AnimationConfig[]): Promise<void> {
		...
		return new Promise((resolve) => {
			...
			configs.forEach((config) => {
				...
				const hasPosition =
					config.position && Object.keys(config.position).length > 0;
				const hasRotation =
					config.rotation && Object.keys(config.rotation).length > 0;
	...
	isPlaying(): boolean {
		return !this.isDestroyed && this.currentTimeline
			? this.currentTimeline.isActive()
			: false;
	}
	...
	startContinuousAnimation(
		...
		const fetchAndAnimate = async () => {
			if (!this.continuousMode || !this.fetchPointsCallback || this.isDestroyed)
				return;
				...
				if (points.length > 0 && !this.isDestroyed) {
					...
					points.forEach((point) => {
						const config: AnimationConfig = {
							...
							rotation:
								point.rotationX !== undefined ||
								point.rotationY !== undefined ||
								point.rotationZ !== undefined
									? {
											x: point.rotationX,
											y: point.rotationY,
											z: point.rotationZ,
