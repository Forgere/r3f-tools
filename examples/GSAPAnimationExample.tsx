...
function ContinuousAnimatedCube({ offset = [0, 0, 0] }: { offset?: [number, number, number] }) {
  ...
  useEffect(() => {
    if (meshRef.current) {
      ...
      animatorRef.current.startContinuousAnimation(mockFetchAnimationPoints, 1500)
    }
    ...
    if (meshRef.current) {
      ...
      const run = async () => {
        await animator.animateSequence([
          { position: { x: 5, y: 2, z: 0 }, rotation: { y: Math.PI / 4 }, duration: 1.5 },
          { position: { x: 3, y: -1, z: 4 }, rotation: { x: Math.PI / 2, y: Math.PI / 2 }, duration: 2 },
          { position: { x: -2, y: 1, z: 2 }, rotation: { x: Math.PI, y: Math.PI }, duration: 1.8 },
          { position: { x: -4, y: -2, z: -3 }, rotation: { x: Math.PI * 1.5, y: Math.PI * 1.5 }, duration: 2.2 },
          { position: { x: 0, y: 0, z: 0 }, rotation: { x: 0, y: 0, z: 0 }, duration: 2 }
        ])
        animator.destroy()
      }
      run()
    ...
    <div style={{ width: '100vw', height: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: '14px', lineHeight: '1.4' }}>
          <p><strong style={{ color: 'orange' }}>Orange Cubes:</strong> Dynamic number of cubes (leva)</p>
          <p><strong style={{ color: 'cyan' }}>Cyan Sphere:</strong> Predefined animation sequence</p>
          <p><strong style={{ color: 'magenta' }}>Magenta Torus:</strong> Manual + periodic animation points</p>
        </div>
      </div>
    </div>
  )
