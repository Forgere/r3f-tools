...
import { moveAlongPath, createPathAnimator, PathAnimator } from '../src/utils/moveAlongPath'
  ...
  return (
    <>
      {positions.map((pos, index) => (
        <SinglePathCube key={index} initialPosition={pos as [number, number, number]} delay={index * 0.5} />
      ))}
    </>
  )
}

function SinglePathCube({ initialPosition, delay }: { initialPosition: [number, number, number], delay: number }) {
  const meshRef = useRef<Mesh>(null)
  ...
  useEffect(() => {
    if (meshRef.current) {
      ...
      const pathPositions = [
        ...
        { x: initialPosition[0] + 1, y: initialPosition[1] + 2, z: initialPosition[2] + 1 },
        { x: initialPosition[0], y: initialPosition[1] + 3, z: initialPosition[2] + 2 },
        { x: initialPosition[0] - 1, y: initialPosition[1] + 2, z: initialPosition[2] + 1 },
        { x: initialPosition[0], y: initialPosition[1], z: initialPosition[2] }
      ]
...
export default function PathAnimationExample() {
  const { showSimple, showComplex, showInteractive, showSynchronized } = useControls('Path Animations', {
    showSimple: true,
    showComplex: true,
  ...
  return (
    <div style={{ width: '100vw', height: '100vh' }}>
      <Canvas camera={{ position: [10, 8, 10], fov: 60 }}>
        ...
        {showSimple && <SimplePathCube />}
        {showComplex && <ComplexPathSphere />}
        {showInteractive && <InteractivePathTorus />}
        {showSynchronized && <SynchronizedCubes />}
      ...
      <div style={{
        ...
        <p>Demonstrates the <code>moveAlongPath</code> utility with various path animations:</p>
        
        <div style={{ marginTop: '10px' }}>
          <p><strong style={{ color: '#ff6b6b' }}>Red Cube:</strong> Simple circular path</p>
          <p><strong style={{ color: '#4ecdc4' }}>Cyan Sphere:</strong> Complex 3D spiral path</p>
          <p><strong style={{ color: '#ffe66d' }}>Yellow Torus:</strong> Interactive random path</p>
          <p><strong style={{ color: '#a8e6cf' }}>Green Cubes:</strong> Synchronized path animations</p>
        </div>
