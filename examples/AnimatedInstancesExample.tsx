...
} from "../src/components/InstanceMeshPool";
...
function AnimatedScene() {
	...
	// 处理点击事件 - 改变轨迹类型
	const handleInstanceClick = (_, index: number) => {
		...
		instance.trajectory =
			trajectories[(currentIndex + 1) % trajectories.length];

		// 重置相位避免突跳
		instance.phase = timeRef.current * instance.speed;
		console.log(
			`Instance ${index} changed to ${instance.trajectory} trajectory`,
		);
