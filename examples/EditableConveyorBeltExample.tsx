...
function EditableConveyorBeltScene() {
	...
	const {
	...
	} = useControls(
		{
			...
			addPointAfter: button(() => {
				console.log(
					"Add point before clicked, selected index:",
					selectedPointIndex,
				);
				if (selectedPointIndex === null) return;

				// Generate a new point along the curve instead of using fixed offset
				const newPoint = generateNewPoint(selectedPointIndex + 1, true);

				const newPoints = [...points];
				// Insert at the current index for "before"
				newPoints.splice(selectedPointIndex + 1, 0, newPoint);
				setPoints(newPoints);

				// Update selected index since we inserted before the current point
				setSelectedPointIndex((prev) => (prev !== null ? prev + 1 : prev));
			}),
			addPointBefore: button(() => {
				console.log(
					"Add point after clicked, selected index:",
					selectedPointIndex,
				);
				if (selectedPointIndex === null) return;
				...
				const newPoints = [...points];
				// Insert after the current index for "after"
				newPoints.splice(selectedPointIndex, 0, newPoint);
				setPoints(newPoints);
			}),
			deleteSelectedPoint: button(() => {
				console.log(
					"Delete point clicked, selected index:",
					selectedPointIndex,
				);
				if (selectedPointIndex === null) return;
	...
	const handleObjectChange = useCallback(() => {
		// Update the points array with the new position from transform controls
...
export function EditableConveyorBeltExample() {
	return (
		<div style={{ width: "100vw", height: "100vh", background: "#111" }}>
			...
			>
				<h3 style={{ margin: "0 0 10px 0" }}>Editable Conveyor Belt</h3>
